...
#calculates the total equivalent nodal point loading for the entire structure, by summing the equivalent nodal point loadings for the UDLs, Point Loads and Nodal Point Loads on all four beams.
Q_total = (Q1_eq_UDL + Q2_eq_UDL + Q3_eq_UDL + Q4_eq_UDL) + (Q1_eq_PL + Q2_eq_PL + Q3_eq_PL + Q4_eq_PL) + Q_nodal
print(Q_total)
...
#applied UDL/point loads on the structure (equilibrium check).
...
def resultant_local_to_global(udl, pl, L, alpha):
    fy_local_total = udl*L + pl
    c, s = np.cos(alpha), np.sin(alpha)
    fx_global = s * fy_local_total
    fy_global = c * fy_local_total
    return fx_global, fy_global

fx1, fy1 = resultant_local_to_global(UDL1, Point_Load1, L1, alpha1)
fx2, fy2 = resultant_local_to_global(UDL2, Point_Load2, L2, alpha2)
fx3, fy3 = resultant_local_to_global(UDL3, Point_Load3, L3, alpha3)
fx4, fy4 = resultant_local_to_global(UDL4, Point_Load4, L4, alpha4)
